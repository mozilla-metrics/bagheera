...
public class Rrd4jReporter extends AbstractPollingReporter implements
    ...
    private File getMetricDBPath(MetricName name) {
        File metric_path = new File(this.dataDir, name.getName() + "-" + this.dbUpdateInterval + ".rrd");
        return metric_path;
    }

    private File getMetricGraphPath(MetricName name) {
        File metric_path = new File(this.graphDir, name.getName() + "-" + this.dbUpdateInterval + ".gif");
        return metric_path;
    }

    private RrdDb createDB(MetricName name, DsType metric_type) throws IOException {

        Integer steps_per_min = (int) Math.round(60 / this.dbUpdateInterval);
        Integer steps_per_hour = (int) Math.round(3600 / this.dbUpdateInterval);

        HashMap<Integer, Integer> archives = new HashMap<Integer, Integer>();
        // 60 * 24 * 7 samples of 1min average
        archives.put(steps_per_min, 10080);
        // 24 * 30 samples of 1hr average
        archives.put(steps_per_hour, 720);
        return createDB(name, metric_type, this.dbUpdateInterval, archives);
    }
    ...
    private RrdDb createDB(MetricName name, DsType metricType, Integer stepSize,
        ...
        DsType rrd_metric_type = metricType;
        File metric_path = getMetricDBPath(name);

        // create new DB
        RrdDef rrdDef = new RrdDef(metric_path.toString(), stepSize);
        rrdDef.setStartTime((System.currentTimeMillis() / 1000) - stepSize);
        rrdDef.addDatasource("value", rrd_metric_type, 2 * stepSize, Double.NaN, Double.NaN);
    ...
    public RrdDb getDBHandle(MetricName name) throws IOException {
        ...
        if (metric_path.exists()) {            
            this.rrdMap.put(name.getName(), new RrdDb(metric_path.toString()));
